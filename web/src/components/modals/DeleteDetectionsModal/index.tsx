...
export interface DeleteSavedQueriesModalProps extends ModalProps {
	ids: SavedQuery['id'][];
	onConfirm?: () => void;
}
	...
	const [deleteSavedQueries] = useDeleteSavedQueries({
		update: cache => {
			cache.modify('ROOT_QUERY', {
				listSavedQueries: (savedQueryCacheData, {toReference}) => {
		...
		onCompleted: () => {
			trackEvent({
				event: EventEnum.DeletedSavedQueries,
				src: SrcEnum.SavedQueries,
		...
		onError: error => {
			...
			trackError({event: TrackErrorEnum.FailedToDeleteSavedQueries, src: SrcEnum.SavedQueries});
		},
	});
