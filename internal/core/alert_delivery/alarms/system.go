...
func CloudWatch() map[string][]*cloudwatch.PutMetricAlarmInput {
	alertingAlarms := []*cloudwatch.PutMetricAlarmInput{
		{
			AlarmName:          aws.String(alarms.SystemAlarmName(metrics.SubsystemAlerting, "AlertDeliveryFailure")),
			AlarmDescription:   aws.String("Panther failed to delivery alerts to destinations"),
			ComparisonOperator: aws.String(cloudwatch.ComparisonOperatorGreaterThanThreshold),
			Dimensions: []*cloudwatch.Dimension{
				{Name: aws.String(metrics.SubsystemDimension), Value: aws.String(metrics.SubsystemAlerting)},
				{Name: aws.String(metrics.StatusDimension), Value: aws.String(alertmetrics.StatusErr)},
			},
	...
	return map[string][]*cloudwatch.PutMetricAlarmInput{
		metrics.SubsystemAlerting: alertingAlarms,
	}
}
